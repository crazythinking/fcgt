...
public class UpDownLimitPostTxn implements Invoker, Skippable {
	
	private static final Logger log = LoggerFactory.getLogger(UpDownLimitPostTxn.class);
	...
	public void invoke(FlowContext ctx) {
		...
		//调用微服务
		EveryDayAccountingBean accountLimitBean = ctx.get(EveryDayAccountingBeanKey.class);
		//TODO v数据拆分组装
		 
		ResponseData response = outerLedgerTxnHstSao.accountLimitTxn(accountLimitBean);
//		ResponseData response= JSON.parseObject(result, ResponseData.class);
		ctx.put(ResponseDataKey.class, response);
		String result = JSON.toJSONString(response);
		log.info("授额|提降额记账的结果为："  + result);
