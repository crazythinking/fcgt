...
public class CorrectionInteAccPostTxn implements Invoker, Skippable {

	private static final Logger log = LoggerFactory.getLogger(CorrectionInteAccPostTxn.class);
	...
	public void invoke(FlowContext ctx) {
		...
		//TODO
		//调用微服务
		ResponseData response = accountingSao.correctionInteAccTxn(correctionAccount);
		
		ctx.put(ResponseDataKey.class, response);
		String result = JSON.toJSONString(response);
		log.info("冲正利息计提的结果为："  + result);
	}
