...
public class RepaymentAccountPostTxn implements Invoker, Skippable {
	...
	public void invoke(FlowContext ctx) {
		EveryDayAccountingBean repaymentAccount = ctx.get(EveryDayAccountingBeanKey.class);
		//TODO
		//调用微服务
		ResponseData response = outerLedgerTxnHstSao.repaymentAccountTxn(repaymentAccount);
		
		ctx.put(ResponseDataKey.class, response);
		String result = JSON.toJSONString(response);
		log.info("还款交易的结果为："  + result);
	}
