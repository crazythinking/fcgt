...
public class CorrectionRepaymentBizCheck implements Invoker, Skippable {

	private static final Logger log = LoggerFactory.getLogger(CorrectionRepaymentBizCheck.class);
	...
	public void invoke(FlowContext ctx) {
		...
		CorrectionRepaymentRequestData data = (CorrectionRepaymentRequestData) JSONObject.toJavaObject(jsonObject,CorrectionRepaymentRequestData.class);
		//联合贷数据
		...
		if (data.getUnionData().size() > 0) {
			for (UnionData unionData : data.getUnionData()) {
				...
				union.setSubAcctType(unionData.getSubAcctType());
				union.setOtherAmt(unionData.getOtherAmt());
				union.setStmHist(unionData.getStmHist());
		...
		requestData.setAccountTrading(AccountTradingDef.C_T);
		...
		// SAO
		String result = accountingSao.correctionRepayment(bean);
